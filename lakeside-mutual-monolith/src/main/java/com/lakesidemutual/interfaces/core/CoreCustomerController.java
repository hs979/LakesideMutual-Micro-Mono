package com.lakesidemutual.interfaces.core;
...
import com.lakesidemutual.interfaces.dtos.customer.*;
 ...
 * But such hybrid approach gets messy soon (due to "annotation jungle"). Hence, there is a separate CoreCustomerControllerSOAPAdapter.
 *
 */

// @RestController  -- commented out: internal-only in monolith
...
public class CoreCustomerController {
	...
	private CustomerResponseDto createCustomerResponseDto(CustomerAggregateRoot customer, String fields) {
		...
		Link selfLink = linkTo(
				methodOn(CoreCustomerController.class).getCustomer(customer.getId().toString(), fields))
				.withSelfRel();

		Link updateAddressLink = linkTo(methodOn(CoreCustomerController.class).changeAddress(customer.getId(), null))
	...
	private PaginatedCustomerResponseDto createPaginatedCustomerResponseDto(String filter, Integer limit,
		...
		.add(linkTo(methodOn(CoreCustomerController.class).getCustomers(filter, limit, offset, fields))
				.withSelfRel());

		if (offset > 0) {
			paginatedCustomerResponseDto.add(linkTo(methodOn(CoreCustomerController.class).getCustomers(filter,
					limit, Math.max(0, offset - limit), fields)).withRel("prev"));
		}

		if (offset < size - limit) {
			paginatedCustomerResponseDto.add(linkTo(
					methodOn(CoreCustomerController.class).getCustomers(filter, limit, offset + limit, fields))
					.withRel("next"));
		}
	...
	public ResponseEntity<CustomersResponseDto> getCustomer(
		...
		Link selfLink = linkTo(methodOn(CoreCustomerController.class).getCustomer(ids, fields)).withSelfRel();
